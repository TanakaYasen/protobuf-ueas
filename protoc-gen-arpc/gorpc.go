...
{{ $outer := . }}
{{- range .SvMethods}}
func (s *{{$outer.ServiceName}}) Send{{.MethodName}}(req *{{.ReqMsg}}) {
	var pkg = &Package{}
	pkg.Route = "{{.MethodName}}"
	pkg.Data, _ = proto.Marshal(req)
	data, _ := proto.Marshal(pkg)
	s.connection.SendPackage(data)
}
{{- end}}
{{end}}
`
...
type {{.ServiceName}}DispatcherImpl interface {
	{{- range .SvMethods}}
	{{.MethodName}} ({{if IsNotEmpty .ReqMsg}} {{.ReqMsg}} {{end}}) {{if IsNotEmpty .RespMsg }}{{.RespMsg}}{{ end }}
	{{- end}}
}
...
func genFmtSource(g *protogen.GeneratedFile, pdata *GenRPC, temptext string) {
	buf := bytes.Buffer{}
	...
	err = templ.Execute(writer, pdata)
	if err != nil {
		log.Fatalln(err)
		return
	}
...
func generateGoRpc(gen *protogen.Plugin, file *protogen.File) {
	...
	genFmtSource(g, pdata, hRpcStubCodeGoServer)

	filename = baseFilename + ".client.go"
	g = gen.NewGeneratedFile(filename, file.GoImportPath)

	genFmtSource(g, pdata, hRpcStubCodeGoClient)
}
